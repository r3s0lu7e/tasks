        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div
                ...
                <div class="p-4 sm:p-6 bg-gradient-to-r from-pink-500 to-purple-600 text-white">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                        <div>
                            <h1 class="text-xl sm:text-2xl font-bold">Add Period Entry</h1>
                            <p class="text-pink-100 mt-1 text-sm sm:text-base">Track your menstrual cycle</p>
                        </div>
            ...
            <div
                ...
                <div class="p-4 sm:p-6">
                    <form method="POST" action="{{ route('period-calendar.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <input type="number" id="cycle_length" name="cycle_length"
                                   value="{{ old('cycle_length', 28) }}" min="20" max="40" required
                        ...
                        <div>
                            ...
                            <input type="number" id="period_length" name="period_length"
                                   value="{{ old('period_length', 5) }}" min="1" max="10" required
                        ...
                        <div>
                            ...
                            <select id="flow_intensity" name="flow_intensity" required
                                ...
                                <option value="light" {{ old('flow_intensity') == 'light' ? 'selected' : '' }}>Light
                                </option>
                                <option value="normal" {{ old('flow_intensity') == 'normal' ? 'selected' : '' }}>Normal
                                </option>
                                <option value="heavy" {{ old('flow_intensity') == 'heavy' ? 'selected' : '' }}>Heavy
                                </option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="symptoms" name="symptoms" rows="3" placeholder="e.g., cramps, headache, mood changes..."
                                      class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-pink-500 focus:border-pink-500 dark:bg-gray-700 dark:text-white sm:text-sm @error('symptoms') border-red-300 @enderror">{{ old('symptoms') }}</textarea>
                            @error('symptoms')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" rows="3" placeholder="Additional notes about this period..."
                                      class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-pink-500 focus:border-pink-500 dark:bg-gray-700 dark:text-white sm:text-sm @error('notes') border-red-300 @enderror">{{ old('notes') }}</textarea>
                            @error('notes')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
